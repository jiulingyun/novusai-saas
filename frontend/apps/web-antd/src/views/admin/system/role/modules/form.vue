...
async function getParentAllowedIds(pid: number): Promise<number[]> {
  ...
  try {
    const detail = await admin.getRoleDetailApi(pid);
...
const [Drawer, drawerApi] = useVbenDrawer({
  ...
  async onOpenChange(isOpen) {
    if (isOpen) {
      ...
      if (isEdit.value && roleId.value) {
        try {
          const detail = await admin.getRoleDetailApi(roleId.value);
...
async function loadRoleList() {
  ...
  try {
    roleList.value = await admin.getRoleListApi();
  } catch {
    roleList.value = [];
  } finally {
...
async function loadPermissions() {
  ...
  try {
    const rawPermissions = await admin.getPermissionTreeApi();
    ...
    <Form>
      <template #parent_id>
        <TreeSelect
          ...
          :placeholder="$t('admin.system.role.placeholder.selectParent')"
          :allow-clear="true"
